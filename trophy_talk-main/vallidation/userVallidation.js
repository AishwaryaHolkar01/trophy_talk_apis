...
const supportVallidation = [
    body("email").optional().isEmail().withMessage("Invalid email format"),
    // body("phoneNumber").optional().isMobilePhone().withMessage("Invalid phone number"),
    body("message").notEmpty().withMessage("Message is required"),
];

const ranchApplicationValidation = [
    // Personal details
    // body('full_name')
    //     .trim()
    //     .notEmpty().withMessage('Full name is required.')
    //     .isLength({ min: 2, max: 200 }).withMessage('Full name must be 3–100 characters.'),
    ...
    body('contact_phone')
        .optional()
        .matches(/^\+?[0-9\s\-]{7,20}$/).withMessage('Provide a valid phone number with country code.'),


    // body('ownership_structure')
    //     .notEmpty().withMessage('Preferred ownership structure is required.'),

    // body('investing_as')
    //     .notEmpty().withMessage('Investing as (individual/entity/trust) is required.')
    //     .isIn(['individual', 'entity', 'trust', 'other']).withMessage('Invalid investing_as value.'),

    // Accreditation & compliance
    body('accredited')
        .notEmpty().withMessage('Accredited investor status is required.')
        .isBoolean().withMessage('Accredited must be true or false.'),

    body('background_felony')
        .notEmpty().withMessage('Background felony flag is required.')
        .isBoolean().withMessage('Background felony must be true or false.'),

    body('pep')
        .notEmpty().withMessage('Politically exposed person (PEP) flag is required.')
        .isBoolean().withMessage('PEP must be true or false.'),

    body('investing_on_behalf')
        .notEmpty().withMessage('Investing on behalf flag is required.')
        .isBoolean().withMessage('Investing on behalf must be true or false.'),

    // // Disclosures & signature
    // body('disclosures')
    //     .notEmpty().withMessage('Disclosures acknowledgement is required.')
    //     .isObject().withMessage('Disclosures must be an object of checkbox confirmations.'),
